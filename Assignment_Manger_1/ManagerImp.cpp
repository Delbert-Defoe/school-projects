...
/*
int lastindex is passed as a reference parameters into functions where it is subject to change to prevent any errors.
it is also passed by value used without being a parameter in functions where it is not subject to change, to prevent errors.
*/
using namespace std;
...
//Case 7 is the exit message, as when the function returns 6 the program will end
case '7':
...
//the table labels are printed first, then all assignments are printed in the for loop

cout<<"---------------------------------------------------------------------------------------------------------------------------------------------------------------"<<endl;
std::cout<<std::setw(10)<<"Course"<<std::setw(10)<<"|"<<std::setw(14)<<"Assignment"<<std::setw(14)<<"|"<<std::setw(15)<<"Due Date"<<std::setw(7)<<"|"<<std::setw(50)<<"Description"<<std::setw(39)<<"|"<<endl;
cout<<"---------------------------------------------------------------------------------------------------------------------------------------------------------------"<<endl;
...
for (int x=0; x<ArraySize; x++){
 ...
 if(lastIndex==x)
    ...
    std::cout<<newAssignment[x].Course_code<<setw(20-newAssignment[x].Course_code.length())<<"|"<<newAssignment[x].Name<<setw(28-newAssignment[x].Name.length())<<"|"<<newAssignment[x].Due_Date<<setw(22-newAssignment[x].Due_Date.length())<<"|"<<newAssignment[x].Description<<setw(89-newAssignment[x].Description.length())<<"|"<<endl;
...
//every time a course is encountered, the counter will be incremented
    y++;
    }


}
//if there are no assignments found with the code, the user will be notified
if(y==0)
 cout<<"No courses have code '"<<coursecode<<"'"<<endl;
...
cout<<"~WELCOME TO ASSIGNMENT MANGER! This application helps you to keep your assignments in order.~\n"
    <<"~To get started you can add an assignment using option 1.~\n"
    <<"~If you made a mistake, or some information was changed, you can simply update that assignment using option 2.~\n"
    <<"~If you would like to remove an assignment all together you may do so using option 3.~\n"
    <<"~You can display all your assignments using option 4, and you can display all assignments with a specific course code using 5.~\n"
    <<"~IT IS BEST TO USE OPTION 4 IN FULL SCREEN MODE, OTHERWISE THE TABLE WILL BE DISFIGURED~\n"
    <<"~Choosing option 7 exits the program. I hope you find this program very useful.~\n"
    <<"\n\n IF YOU DELETE AN ASSIGNMENT, PLEASE DO NOT ADD ANOTHER ASSIGNMENT UNTIL YOU RESTART THE PROGRAM\n";
